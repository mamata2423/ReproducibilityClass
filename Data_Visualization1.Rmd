...
```{r}
#create bar chart with standard-error bars
ggplot(datum, aes(x=Treatment, y= DON, color= Cultivar)) +
...
```{r}
#Add points to the foreground of boxplot with distribution of points making shape 21 and outline color black
ggplot(datum, aes(x=Treatment, y=DON, color=Cultivar)) +
         geom_boxplot() +
...
#Add points to the foreground of barchart with distribution of points making shape 21 and outline color black
ggplot(datum, aes(x=Treatment, y= DON, color= Cultivar)) +
  stat_summary(fun=mean, geom="bar") +
  stat_summary(fun.data = mean_se, geom = "errorbar") +
  geom_point(position=position_jitterdodge(dodge.width = 0.9), aes(fill=Cultivar), shape=21, colour="black")+
...
```{r}
#Change fill color of points and boxplots to match with colorblind pallet
...
#Change fill color of points and barchart to match with colorblind pallet
ggplot(datum, aes(x=Treatment, y= DON, fill=Cultivar)) +
  stat_summary(fun=mean, geom="bar") +
  stat_summary(fun.data = mean_se, geom = "errorbar") +
  geom_point(position=position_jitterdodge(dodge.width = 0.9), aes(fill=Cultivar), shape=21, colour="black")+
...
```{r}
#Add a facet to the boxplot based on cultivar
cbbPalette <- c( "#009E73", "#CC79A7")
ggplot(datum, aes(x=Treatment, y=DON, fill=Cultivar)) +
         geom_boxplot() +
  geom_point(position=position_jitterdodge(dodge.width = 0.9), aes(fill=Cultivar), shape=21, colour="black")+
...
#Add a facet to the barchart based on cultivar
ggplot(datum, aes(x=Treatment, y= DON, fill=Cultivar)) +
  stat_summary(fun=mean, geom="bar") +
  stat_summary(fun.data = mean_se, geom = "errorbar") +
  geom_point(position=position_jitterdodge(dodge.width = 0.9), aes(fill=Cultivar), shape=21, colour="black")+
...
Link to my github repository: https://github.com/mamata2423/ReproducibilityClass/blob/main/Data_visualization.R
...
Link to my partner's repository: https://github.com/mamata2423/Class_Reproducibility.git
